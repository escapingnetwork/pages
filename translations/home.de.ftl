navbar-home = Startseite
navbar-services = Unsere Dienstleistungen
navbar-host = Werden Sie Gastgeber
navbar-partner = Werden Sie Partner
navbar-about = Über Uns
navbar-howitworks = Wie es funktioniert

home-headline = Auf der Suche nach einer Unterkunft in Dublin?
home-subline = Hier bei Capybara House konzentrieren wir uns darauf, die besten Unterkunftsmöglichkeiten in Irland anzubieten.

button-request-accommodation = Unterkunft anfragen
button-services = Unsere Dienstleistungen

footer-contact = Kontakt
footer-support = Unterstützung
footer-meeting = Termin vereinbaren

footer-legal = Rechtliches
footer-privacy-policy = Datenschutzerklärung
footer-terms-conditions = Allgemeine Geschäftsbedingungen

footer-language = Sprache

footer-copyright = Alle Rechte vorbehalten.

about-title = Über uns
about-who = Wer sind wir?
about-who-content = Wir sind ein Team, das seit Jahren mit internationalen Studenten und dem Unterkunftssektor in Irland zusammenarbeitet und sich auf die Verbesserung der Kundenerfahrung und der Zufriedenheit der Anbieter konzentriert.
about-product = Unser Produkt
about-product-content = Capybara House ist eine Marke von Tranquera LLC, einem US-amerikanischen Unternehmen.
about-where = Warum haben wir unseren Sitz in den USA?
about-where-content = Wir sind ein Unternehmen, das mit internationalen Studenten und irischen Anbietern zusammenarbeitet. Aus diesem Grund sind die USA perfekt für die Führung unseres internationalen Geschäfts.
about-meet-team = Lernen Sie unser Team kennen
about-meet-team-content = Unser Team besteht aus engagierten Fachleuten, die daran arbeiten, die beste Erfahrung für unsere Kunden zu gewährleisten.

services-title = Unsere Dienstleistungen
services-half-board = Halbpension
services-self-catering = Selbstverpflegung
services-hostels = Hostels
services-readmore = Mehr erfahren

forms-forename = Vorname
forms-surname = Nachname
forms-email = E-Mail
forms-phone = Telefonnummer
forms-message = Nachricht
forms-country = Land
forms-age = Alter
forms-sex = Geschlecht
forms-sex-male = Männlich
forms-sex-female = Feminin
forms-sex-other = Andere
forms-institution = Institution
forms-service = Dienstleistung
forms-from = Von
forms-to = Bis
forms-contact = Bevorzugte Kontaktmethode
forms-submit = Absenden
forms-submitting = Senden...
forms-error-required = Erforderlich
forms-error-captcha = Falsches Captcha
forms-error = Beim Senden Ihrer Anfrage ist ein Fehler aufgetreten!
forms-error-contact = Bitte kontaktieren Sie
forms-success = Danke
forms-success-contact = Wir melden uns bald bei Ihnen zurück!

forms-rating-capybara = Bewerten Sie Ihre Erfahrung mit Capybara House
forms-rating-user = Bewerten Sie Ihre Erfahrung mit dem Studenten/Gastgeber
forms-content-capybara = Teilen Sie eine Bewertung über Capybara House
forms-content-user = Teilen Sie eine Bewertung über den Studenten/Gastgeber
forms-rating-very-unsatisfied = Sehr unzufrieden
forms-rating-very-satisfied = Sehr zufrieden
forms-review-success = Vielen Dank für Ihre Bewertung!
forms-review-message = Wir schätzen Ihr Feedback.

reviews-title = Bewertungen
reviews-subtitle = Was unsere Kunden über uns sagen
reviews-see-more = Mehr anzeigen
reviews-user-type-student = Student
reviews-user-type-host = Gastgeber
reviews-translated = Übersetzt aus

languages-en = Englisch
languages-de = Deutsch
languages-es = Spanisch
languages-pt = Portugiesisch
languages-it = Italienisch
languages-fr = Französisch

seo-home-title = Capybara House - Startseite
seo-home-description = Finden Sie die besten Unterkunftsmöglichkeiten in Dublin mit Capybara House.

seo-services-title = Capybara House - Unsere Dienstleistungen
seo-services-description = Entdecken Sie die von Capybara House angebotenen Dienstleistungen für Ihren Aufenthalt in Dublin.

seo-host-title = Capybara House - Werden Sie Gastgeber
seo-host-description = Erfahren Sie, wie Sie Gastgeber bei Capybara House werden und internationale Studenten beherbergen können.

seo-partner-title = Capybara House - Werden Sie Partner
seo-partner-description = Werden Sie Partner von Capybara House und helfen Sie, die besten Unterkünfte in Dublin bereitzustellen.

seo-about-title = Capybara House - Über Uns
seo-about-description = Erfahren Sie mehr über Capybara House und unsere Mission, das Unterkunftserlebnis in Irland zu verbessern.

seo-support-title = Capybara House - Unterstützung
seo-support-description = Erhalten Sie Unterstützung und Hilfe vom Capybara House-Team.

seo-reviews-title = Capybara House - Bewertungen
seo-reviews-description = Lesen Sie Kundenbewertungen über Capybara House.

seo-request-accommodation-title = Capybara House - Unterkunft anfragen
seo-request-accommodation-description = Fragen Sie eine Unterkunft bei Capybara House an und finden Sie den perfekten Ort zum Übernachten in Dublin.

seo-privacy-policy-title = Capybara House - Datenschutzerklärung
seo-privacy-policy-description = Lesen Sie die Datenschutzerklärung von Capybara House, um zu erfahren, wie wir Ihre Informationen schützen.

seo-booking-terms-title = Capybara House - Buchungsbedingungen
seo-booking-terms-description = Lesen Sie die Buchungsbedingungen von Capybara House, um unsere Buchungsrichtlinien zu verstehen.

seo-how-it-works-title = Capybara House - Wie es funktioniert
seo-how-it-works-description = Erfahren Sie, wie Capybara House funktioniert und wie wir Ihnen helfen können, die beste Unterkunft in Dublin zu finden.