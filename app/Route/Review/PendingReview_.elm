...
rtingToString : RatingEnum -> String
rtingToString rting =
    case rting of
        One ->
            "1"
        ...
        (\id content rating captcha hiddenCaptcha ->
            ...
            , view =
                \formState ->
                    let
                        ...
                        fieldViewRating label field =
                            Html.div [ Attrs.class "mb-5" ]
                                [ Html.label [ Attrs.class "block mb-2 text-lg font-semibold text-gray-900 dark:text-white" ]
                                    [ Html.text (label ++ " ")
                                    , Form.FieldView.radio [ Attrs.class "flex space-x-4 mt-2" ]
                                        (\enum toRadio ->
                                            Html.div [ Attrs.class "flex items-center" ]
                                                [ toRadio [ Attrs.class "" ]
                                                , Html.label [ Attrs.class "flex items-center cursor-pointer peer-checked:bg-primary-500 peer-checked:text-white p-2 rounded-lg border border-gray-300 dark:border-gray-600" ]
                                                    [ Html.span [ Attrs.class "mr-2" ]
                                                        [ Html.text (rtingToString enum)
                                                        ]
                                                    , case enum of
                                                        One ->
                                                            ReviewUtils.ratingToStars 1

                                                        Two ->
                                                            ReviewUtils.ratingToStars 2

                                                        Three ->
                                                            ReviewUtils.ratingToStars 3

                                                        Four ->
                                                            ReviewUtils.ratingToStars 4

                                                        Five ->
                                                            ReviewUtils.ratingToStars 5
                                                    ]
                                                ]
                                        )
                                        field
                    ...
                    in
                    [ fieldView (Translations.formsContent t) content
                    , fieldViewRating (Translations.formsRating t) rating
        ...
        [ Html.div [ Attrs.class "mx-auto prose max-w-none pb-8 pt-8 dark:prose-invert xl:col-span-2 xl:max-w-5xl xl:px-0" ]
            ...
            , if (app.data.pendingReview |> .status) /= ReviewUtils.ReviewStatusPending then
                Html.div [ Attrs.class "text-center" ]
                    [ Html.h2 [] [ Html.text "Thank you for your review!" ]
                    , Html.p [] [ Html.text "We appreciate your feedback." ]
                    ]
